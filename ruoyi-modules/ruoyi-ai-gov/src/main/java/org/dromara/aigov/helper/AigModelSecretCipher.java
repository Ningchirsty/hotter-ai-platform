 ...
 * 因此本类在启用时会在启动阶段做一次「加密→解密」自检并<b>快速失败</b>。</p>
 *
...
public class AigModelSecretCipher {
    ...
    /**
     * 实际的 SM4 实例；{@link AigModelCryptoProperties#isEnabled()} 为 false 时为 null。
     */
    private SM4 sm4;

    /**
     * 启动自检：启用时校验参数合法性并验证加解密闭环。
    ...
    public void init() {
        if (!properties.isEnabled()) {
            log.info("模型密钥录入未启用（aigov.model-crypto.enabled=false）：密钥仍只在 snail-ai 管理端配置");
            return;
        }
        ...
        log.info("模型密钥录入已启用（SM4/CBC/PKCS5Padding），配置指纹 fingerprint={}；"
            + "请与 snail-ai 侧日志比对，不一致则密钥密文无法被解密", fingerprint());
    }
    ...
    public String encrypt(String plainApiKey) {
        ...
        if (sm4 == null) {
            throw new ServiceException("模型密钥录入未启用，无法写入密钥（请配置 aigov.model-crypto.enabled=true）");
        }
