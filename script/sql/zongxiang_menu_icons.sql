...
--       里并不存在，侧栏会渲染成空白：
--         业务应用 grid / 管理中心 setting / 平台治理 robot / 平台配置 set-up
--       另外 AI会话、我的公文、敏感操作审计、参数设置、组织与权限 的图标语义不够贴切。
--
-- 注意：旧模块 ruoyi-talent 已退役，「重复人才预警」（1762000000000000103）随之下线，
--       本文件已移除对它的引用；「敏感操作审计」改指向招聘与人才管理模块的菜单
--       1766000000000000113（hr_talent_menu.sql 建立，component hrtalent/audit/index）。
--
...
update sys_menu set icon = 'eye-open'     where menu_id = 1766000000000000113 and icon <> 'eye-open';     -- 敏感操作审计（审计查看，招聘与人才管理模块）
...
select menu_id, menu_name, icon from sys_menu
 where menu_id in (1764000000000000001, 1764000000000000002, 1764000000000000003, 1764000000000000004,
                   ...
                   1761400000000000008, 1761400000011629, 1766000000000000113,
