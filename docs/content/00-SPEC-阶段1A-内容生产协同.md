...
## 12. 增补：任务表单 ↔ 闸门字段链路（产品信息带入 + 录入引导）

> 本节为实施后增补。起因是两处被实际使用后暴露的问题：**选中产品后产品与SKU模块的信息没有被带过来**，
> 以及**任务表单字段很少、而闸门要求 7 个强制事实，用户不知道这些值从哪进**。
...
| 层 | 表 | 角色 | 闸门依据 |
|---|---|---|---|
| 产品标识 | `cp_product` | 轻量产品/SKU，只做可追溯标识（B5） | 否 |
| 任务元数据 | `cp_task` | 任务名/交付类型/产品/截止/负责人/敏感级别 | 否 |
| 产品事实 | `cp_fact_snapshot` | 候选值 + 人工确认值 | **是，唯一依据** |

因此「选中产品 → 闸门自动满足」在数据上**不可能**：闸门要的 `main_version`/`color`/`quantity`/
`spec_params`/`package_version` 在 `cp_product` 里**根本没有对应列**，只能来自产品资料（解析+确认）
或人工录入。任务表单字段少是刻意的——事实只能来自经确认的产品资料（红线第 2 条）。

### 12.2 两个真实缺陷（已修）

| # | 现象 | 根因 |
|---|---|---|
| 1 | 选中产品后 `cp_task.sku_code` 仍为空 | 表单 placeholder 承诺「可留空，默认取产品的SKU」，但**前后端都没有实现这个默认值** |
| 2 | 手工录入 `product_height`（界面提示里的示例）**录入成功但闸门纹丝不动** | `addManual` 不校验字段编码；而 `product_height` 根本不在别名表里。闸门只认与 `cp_gate_rule.field_code` 完全一致的编码 → **静默无效**，且 `ContentFieldAlias.known()` 是死代码 |

另有一个连带缺陷：`cp_task.task_no` 与 `cp_output_check.check_no` 是同一类坑——
**唯一索引不含 `del_flag` + 逻辑删除**，当天记录被软删后序号从 0001 重来、插入撞唯一索引，
表现为「当天把任务删完之后，当天再也建不了新任务」。`check_no` 已在 §11 修复，`task_no` 本次按同一口径修复。

### 12.3 改进内容

1. **A1 选中产品即带出信息**：前端选中产品自动填 SKU（不覆盖已手填值），服务端在 `create()` 里同样兜底
   （保证接口调用方行为一致），并在表单里只读展示该产品的品牌/二级分类/产品经理。
2. **A2 字段编码改为下拉**：新增 `GET /content/fact/fieldOptions?taskId=`，返回「本交付类型的闸门要求项
   （带中文名、闸门等级、是否已满足）」+「别名表里其他已登记字段」。前端分组展示，另留「改为自定义编码」。
3. **A3 服务端校验编码**：可录入 = 别名表已登记 ∪ 存在启用中的闸门规则（取并集，因为闸门规则是表驱动、
   运营可改的）。未登记编码直接拒绝，并在提示里列出本交付类型要求的事实字段——**把静默无效变成明确报错**。
4. **A4 闸门表直达录入**：未满足的强制项/条件项表格加「录入该字段」按钮，编码与中文名预填，不用用户抄编码。
5. **B1 产品主数据 → 产品事实**（本次新增的边界，见下）。

### 12.4 B1 的边界（重要）

选中/挂上产品时，把产品与SKU主数据的**名称与 SKU** 写成产品事实：

- **只同步主数据确实拥有的两个字段**（`product_name`、`sku_code`）。这既解决了"选了产品还要再录一遍"，
  又没有把红线撑开——其余 5 个强制项在 `cp_product` 里本就没有对应列。
- **来源可追溯**：`source_locator` 标注「产品与SKU主数据 · 产品编码 XXX」，`remark` 说明效力边界
  （如与该产品资料不一致，以资料确认结果为准）。
- **不覆盖人的判断**：同字段已存在**不同取值**时，本次写入降级为 `PENDING`（冲突候选），交人裁定；
  已确认的人工值保持 `CONFIRMED` 不被改写。
- **幂等**：取值已一致则跳过，不重复写入。
- **不改任务状态**：创建/编辑任务时只写事实，不触发闸门重算——任务状态只能经「闸门重算」这一条规则入口流转
  （§4.1）。显式同步接口（人工动作）才重算闸门。

> 该边界的依据：`cp_product` 不是「参考图或 AI 输出」，而是人在受治理模块里维护的主数据，
> 属于红线第 2 条所说的「经确认的产品资料」的一种。**若后续判定它不算，只需去掉 §12.4 即可，
> 其余改进（A1–A4）与该判断无关。**

### 12.5 验收方式

1. 选中产品、SKU 留空建任务 → 断言 `cp_task.sku_code` 等于产品的 SKU
2. 断言 `product_name` / `sku_code` 两条事实为 `CONFIRMED`，`source_locator` 含「产品与SKU主数据」
3. 断言闸门未满足强制项由 7 项降到 5 项，且剩下的正是产品表没有的字段
4. 手工录入未登记编码（如 `product_height`、`sku`）→ 断言**被拒绝**且提示里列出要求字段
5. 已有不同取值时同步 → 断言人工值仍 `CONFIRMED`、主数据值只落 `PENDING`
6. **回归**：删光当天全部任务后再建任务 → 断言成功且任务号顺延（不再 409）
