...
-- ----------------------------------------------------------------------------
-- 位置说明（重要）：
--   本文件**刻意不放在 script/sql/**。那个目录是部署脚本目录，容易被按
--   「把 script/sql 下的 SQL 全跑一遍」的方式批量执行。本夹具一旦误入生产库，
--   会在 sai_model_config 插入一条假模型并把 talent_match 绑到它上面，
--   使该能力路由到一个并不存在的本地模型——审计上却显示为「本地未外发」，
--   属于看起来合规、实际不可用的隐蔽故障。故移到 docs/ 下，与部署解耦。
--
